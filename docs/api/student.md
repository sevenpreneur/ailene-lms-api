...
- `prompting_quality` — mean of the 5-part rubric (`rubric_specificity`/`context`/`constraints`/`examples`/`iteration`) across the caller's *reviewed* `lms_prompt_submissions` in the last 90 days; if fewer than 3 fall in that window, falls back to the 3 most recently reviewed overall (submissions missing any rubric value are skipped entirely).
